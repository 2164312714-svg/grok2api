...
func TestGenerateWSImageReacquiresAfterChallengeHandshake(t *testing.T) {
	...
	adapter := NewAdapter(Config{BaseURL: server.URL}, infraegress.NewManager(egressRepositoryStub{}, cipher), cipher, nil, imageAssetStoreStub{})
	response, err := adapter.GenerateImage(context.Background(), provider.ImageGenerationRequest{
		...
		Model:      "grok-imagine-image-quality", Prompt: "draw a teapot", Count: 1, ResponseFormat: "b64_json",
	})
	if err != nil {
		t.Fatal(err)
	}
