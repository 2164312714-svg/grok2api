...
func main() {
	...
	mux.HandleFunc("/healthz", requireMethod(http.MethodGet, healthHandler))
	mux.HandleFunc("/sign", requireMethod(http.MethodPost, signHandler))
	server := &http.Server{Addr: listenAddress, Handler: mux, ReadHeaderTimeout: 5 * time.Second, ReadTimeout: 10 * time.Second, WriteTimeout: 10 * time.Second, IdleTimeout: 30 * time.Second}
...
func generateStatsig(pathname, method string, nowUnix int64) (string, error) {
	...
	if method == "" || strings.IndexFunc(method, func(value rune) bool { return value < 'A' || value > 'Z' }) >= 0 {
